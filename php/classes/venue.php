...
class Venue
{
	...
	public static function getVenueByVenueId(&$mysqli, $venueId)
	{
		...
		// create query template
		$query = "SELECT venueId, venueName, venueCapacity, venuePhone, venueWebsite, venueAddress1, venueAddress2, venueCity, venueState, venueZipCode FROM venue WHERE venueId = ?";
		$statement = $mysqli->prepare($query);
		if($statement === false) {
			throw(new mysqli_sql_exception("Unable to prepare statement"));
		}
...
//		$row = $result->fetch_assoc(); // fetch_assoc returns a row as an associative array
	...
	public static function getVenueByVenueZipCode(&$mysqli, $venueZipCode)
	{
		...
		// 1) if there's more than 1 result, we can make all into Venue objects
		// 2) if there's no result, we can just return null
		// $row = $results->fetch_assoc(); // fetch_assoc returns rows as an associative array
