...
class UserTest extends UnitTestCase
{
	...
	public function testDeleteUserId() {
		...
		$hopefulUserId = UserId::getUserIdByEmail($this->mysqli, $this->EMAIL);
	...
	public function testGetUserIdByEmail() {
		...
		$staticUser = UserId::getUserByEmail($this->mysqli, $this->EMAIL);

		// finally, compare the fields
		$this->assertNotNull($staticUser->getUserId());
		$this->assertTrue($staticUser->getUserId() > 0);
		$this->assertIdentical($staticUser->getUserId(),              $this->userId->getUserId());
